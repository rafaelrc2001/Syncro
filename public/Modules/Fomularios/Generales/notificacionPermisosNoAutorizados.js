// Función para manejar el envío de notificación de permisos NO autorizados a n8n
window.notificacionPermisosNoAutorizadosHandler = async function () {
	...
	if (depElem) {
		...
		try {
			...
			console.warn('[notificacionPermisosNoAutorizadosHandler] No se pudo obtener departamento de localStorage');
		}
	}
	...
	if (solicitante) {
		try {
			...
			console.warn('[notificacionPermisosNoAutorizadosHandler] Error obteniendo correo del solicitante:', e);
		}
	}
	...
	try {
		...
		console.warn('[notificacionPermisosNoAutorizadosHandler] Error obteniendo correos de supervisores:', e);
	}
	...
	const datosPermisoNoAutorizado = {
		id_permiso,
		sucursal,
		area,
		descripcion,
	...
	try {
		const response = await fetch(
			urlWebhook,
			{
				...
				body: JSON.stringify(datosPermisoNoAutorizado),
			}
		);
		if (!response.ok) {
