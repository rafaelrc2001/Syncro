...
function initMaintenanceTypeToggle() {
  const otherMaintenanceRadio = document.getElementById(
    "other-maintenance-radio"
  );
  const otherMaintenanceTypeInput = document.getElementById(
  ...
  if (otherMaintenanceRadio && otherMaintenanceTypeInput) {
    ...
    document
      .querySelectorAll('input[name="maintenance-type"]:not([value="OTRO"])')
      .forEach((radio) => {
...
function initEquipmentHandling() {
  ...
  const equipmentDescriptionInput = document.getElementById(
  ...
  if (hasEquipmentYes && equipmentDescriptionInput) {
    ...
    const hasEquipmentNo = document.querySelector(
      'input[name="has-equipment"][value="no"]'
    );
    if (hasEquipmentNo) {
  ...
  if (equipmentToggle && equipmentConditions) {
    equipmentToggle.addEventListener("change", function (event) {
      if (event.target.name === "has-equipment") {
        equipmentConditions.style.display =
          event.target.value === "no" ? "none" : "block";
      }
    });
...
function initRiskManagementToggles() {
  ...
  const fireProtectionRadios = document.querySelectorAll(
    'input[name="fire-protection"]'
  );
  const fireProtectionTypeInput = document.getElementById(
  ...
  if (fireProtectionRadios.length && fireProtectionTypeInput) {
    ...
    // Estado inicial
    const selectedFireProtection = document.querySelector(
      'input[name="fire-protection"]:checked'
    );
  ...
  const specialToolsRadios = document.querySelectorAll(
    'input[name="special-tools"]'
  );
  ...
  if (specialToolsRadios.length && specialToolsTypeInput) {
    ...
    // Estado inicial
    const selectedSpecialTools = document.querySelector(
      'input[name="special-tools"]:checked'
    );
