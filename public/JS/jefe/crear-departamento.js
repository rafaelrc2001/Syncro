document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderizarTablaDepartamentos() {
    ...
    if (terminoBusqueda) {
      ...
      filtradas = departamentosGlobal.filter(
        (dep) =>
  ...
  // --- MODAL DE ADVERTENCIA PARA ELIMINAR DEPARTAMENTO ---
    ...
    departmentForm.addEventListener("submit", async function (e) {
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.error || "Error al guardar el departamento"
          );
        }
