...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (btnNoAutorizar) {
    btnNoAutorizar.addEventListener("click", function () {
      ...
      if (noModal) {
        // Rellenar datos del modal de no autorización (igual que el modal de autorización)
        try {
...
// --- Lógica para el botón "Autorizar" con modal de confirmación (adaptada para PT10) ---
document.addEventListener("DOMContentLoaded", function () {
...
// Función para ejecutar la autorización de PT10
async function ejecutarAutorizacionPT10() {
  ...
  try {
    let idEstatus = null;
    console.log("[DEPURACIÓN PT10] Consultando permiso con ID:", idPermiso);
    
    const respEstatus = await fetch(`/api/permisos-trabajo/${idPermiso}`);
    console.log("[DEPURACIÓN PT10] Status de consulta permiso:", respEstatus.status);
    
    if (respEstatus.ok) {
      const dataEstatus = await respEstatus.json();
      console.log("[DEPURACIÓN PT10] Datos completos del permiso:", dataEstatus);
      ...
      if (dataEstatus && dataEstatus.data && dataEstatus.data.id_estatus) {
        ...
        console.log("[DEPURACIÓN PT10] id_estatus encontrado en dataEstatus.data.id_estatus:", idEstatus);
      } else if (dataEstatus && dataEstatus.id_estatus) {
        idEstatus = dataEstatus.id_estatus;
        console.log("[DEPURACIÓN PT10] id_estatus encontrado en dataEstatus.id_estatus:", idEstatus);
      } else if (dataEstatus && dataEstatus.general && dataEstatus.general.id_estatus) {
        idEstatus = dataEstatus.general.id_estatus;
        console.log("[DEPURACIÓN PT10] id_estatus encontrado en dataEstatus.general.id_estatus:", idEstatus);
      } else {
        console.error("[DEPURACIÓN PT10] No se encontró id_estatus en ninguna estructura. Estructura recibida:", Object.keys(dataEstatus || {}));
        
        // Intentar buscar id_estatus en cualquier lugar del objeto
        function buscarIdEstatus(obj, path = '') {
          for (const key in obj) {
            if (key === 'id_estatus' && obj[key]) {
              console.log(`[DEPURACIÓN PT10] id_estatus encontrado en ${path}${key}:`, obj[key]);
              return obj[key];
            }
      ...
      console.error("[DEPURACIÓN PT10] Error HTTP al obtener permiso:", respEstatus.status, respEstatus.statusText);
      alert(`Error al consultar el permiso: ${respEstatus.status} ${respEstatus.statusText}`);
      return;
    }

    if (idEstatus) {
      console.log("[DEPURACIÓN PT10] Actualizando estatus con id_estatus:", idEstatus);
      try {
        const payloadEstatus = { id_estatus: idEstatus };
        console.log("[DEPURACIÓN PT10] Payload para estatus/seguridad:", payloadEstatus);
        ...
        console.log("[DEPURACIÓN PT10] Status de respuesta:", respEstatus.status);
        
        let data = {};
        try {
          data = await respEstatus.json();
          console.log("[DEPURACIÓN PT10] Respuesta del servidor:", data);
        } catch (e) {
          console.error("[DEPURACIÓN PT10] Error al parsear JSON de respuesta:", e);
        }
        
        if (!respEstatus.ok) {
          console.error("[DEPURACIÓN PT10] Error HTTP en estatus/seguridad:", respEstatus.status, data);
          alert(`Error al actualizar estatus: ${data.message || data.error || 'Error desconocido'}`);
          return;
        }
        
        if (data.success === false) {
          console.error("[DEPURACIÓN PT10] API reportó fallo:", data);
          alert(`Error al actualizar estatus: ${data.message || data.error || 'Error desconocido'}`);
          return;
        }
        
        console.log("[DEPURACIÓN PT10] Estatus actualizado exitosamente a 'A espera de Seguridad'");
        
      } catch (err) {
        console.error("[DEPURACIÓN PT10] Excepción al actualizar estatus de seguridad:", err);
        alert(`Error de conexión al actualizar estatus: ${err.message}`);
        return;
      }
    } else {
      console.error("[DEPURACIÓN PT10] No se obtuvo id_estatus para actualizar estatus.");
      alert("No se pudo obtener el ID de estatus del permiso. No se puede autorizar.");
      return;
    }

    // 4. Generar timestamp para autorización
    ...
    console.log("[AUTORIZAR PT10] Timestamp generado (hora local):", fechaHoraAutorizacionArea);
    ...
    // 6. Mostrar modal de confirmación de autorización exitosa
    ...
    // 8. Configurar redirección al cerrar modal
    ...
    console.error("[DEPURACIÓN] Error al autorizar PT10:", err);
  }
}
document.addEventListener("DOMContentLoaded", function () {
