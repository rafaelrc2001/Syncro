        ...
        permitForm.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                if (tipoFormulario === 1) {
                    ...
                    const hora_inicio = `${fecha} ${hora}`;
                    const tiene_equipo_intervenir = document.querySelector('input[name="has-equipment"]:checked')?.value || null;
                    ...
                    const requiere_herramientas_especiales = document.querySelector('input[name="special-tools"]:checked')?.value || null;
                    const tipo_herramientas_especiales = document.getElementById('special-tools-type')?.value || null;
                    const herramientas_adecuadas = document.querySelector('input[name="adequate-tools"]:checked')?.value || null;
                    const requiere_verificacion_previa = document.querySelector('input[name="pre-verification"]:checked')?.value || null;
                    const requiere_conocer_riesgos = document.querySelector('input[name="risk-knowledge"]:checked')?.value || null;
                    const observaciones_medidas = document.getElementById('final-observations')?.value || null;

                    // Requisitos para efectuar el trabajo
                    const fuera_operacion = document.querySelector('input[name="out-of-operation"]:checked')?.value || null;
                    const despresurizado_purgado = document.querySelector('input[name="depressurized"]:checked')?.value || null;
                    const necesita_aislamiento = document.querySelector('input[name="isolated"]:checked')?.value || null;
                    const con_valvulas = document.querySelector('input[name="with-valves"]:checked')?.value || null;
                    const con_juntas_ciegas = document.querySelector('input[name="with-blinds"]:checked')?.value || null;
                    const producto_entrampado = document.querySelector('input[name="trapped-product"]:checked')?.value || null;
                    const requiere_lavado = document.querySelector('input[name="requires-washing"]:checked')?.value || null;
                    const requiere_neutralizado = document.querySelector('input[name="requires-neutralization"]:checked')?.value || null;
                    const requiere_vaporizado = document.querySelector('input[name="requires-steaming"]:checked')?.value || null;
                    const suspender_trabajos_adyacentes = document.querySelector('input[name="adjacent-work-suspended"]:checked')?.value || null;
                    const acordonar_area = document.querySelector('input[name="area-cordoned"]:checked')?.value || null;
                    const prueba_gas_toxico_inflamable = document.querySelector('input[name="gas-test-required"]:checked')?.value || null;
                    const equipo_electrico_desenergizado = document.querySelector('input[name="de-energized"]:checked')?.value || null;
                    const tapar_purgas_drenajes = document.querySelector('input[name="drains-covered"]:checked')?.value || null;

                    // Requisitos para administrar los riesgos
                    const proteccion_especial_recomendada = document.querySelector('input[name="special-protection"]:checked')?.value || null;
                    const proteccion_piel_cuerpo = document.querySelector('input[name="skin-protection"]:checked')?.value || null;
                    const proteccion_respiratoria = document.querySelector('input[name="respiratory-protection"]:checked')?.value || null;
                    const proteccion_ocular = document.querySelector('input[name="eye-protection"]:checked')?.value || null;
                    const proteccion_contraincendio = document.querySelector('input[name="fire-protection"]:checked')?.value || null;
                    const tipo_proteccion_contraincendio = document.getElementById('fire-protection-type')?.value || null;
                    const instalacion_barreras = document.querySelector('input[name="barriers-required"]:checked')?.value || null;
