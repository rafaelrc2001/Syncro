document.addEventListener('DOMContentLoaded', () => {
    ...
    function collectParticipants() {
        ...
        rows.forEach((row, index) => {
            const name = row.querySelector(`[name="participant-name-${index + 1}"]`).value.trim();
            const credential = row.querySelector(`[name="participant-credential-${index + 1}"]`).value.trim();
            const position = row.querySelector(`[name="participant-position-${index + 1}"]`).value.trim();
            const role = row.querySelector(`[name="participant-role-${index + 1}"]`).value;
            ...
            if (!credential) {
                throw new Error(`El número de credencial del participante ${index + 1} es requerido`);
            }
            if (!position) {
                throw new Error(`El cargo del participante ${index + 1} es requerido`);
            }
            if (!role) {
                throw new Error(`Debe seleccionar un rol para el participante ${index + 1}`);
            }
   ...
   // Reemplaza el event listener existente de btn-save-participants con este código
    ...
    function goToNextSection(e) {
        const nextSection = btnSaveParticipants.closest('.form-section').nextElementSibling;
        if (nextSection) {
    ...
    async function handleInsertAndNavigate(e) {
        ...
        try {
            ...
            submitBtn.innerHTML = '<i class="ri-loader-4-line spin"></i> Guardando...';
            ...
            const astData = {
                ...
                maquinaria_equipo_herramientas: document.getElementById('equipment-tools').value.trim(),
                materiales_accesorios: document.getElementById('materials').value.trim()
            };

            // Validar que al menos un campo de AST tenga datos
            if (!astData.epp && !astData.maquinaria_equipo_herramientas && !astData.materiales_accesorios) {
                throw new Error('Debe completar al menos un campo de EPP, Equipos o Materiales');
            }

            const astResponse = await fetch('http://localhost:3000/api/ast', {
                method: 'POST',
                headers: { 
                    'Content-Type': 'application/json',
                    'Accept': 'application/json'
            ...
            if (btnSaveParticipants) {
                ...
                btnSaveParticipants.innerHTML = 'Siguiente <i class="ri-arrow-right-line"></i>';
            }
        }
    }
...
document.getElementById('btn-save-ast')?.addEventListener('click', async (e) => {
    ...
    try {
        ...
        const response = await fetch('http://localhost:3000/api/ast', {
            method: 'POST',
            headers: { 
                'Content-Type': 'application/json',
                'Accept': 'application/json'
        ...
        // Avanzar al siguiente paso
        const nextSection = e.target.closest('.form-section').nextElementSibling;
        if (nextSection) {
            nextSection.classList.add('active');
            nextSection.scrollIntoView({ behavior: 'smooth' });
        }
