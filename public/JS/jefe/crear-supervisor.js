document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderizarTablaSupervisores() {
    ...
    if (terminoBusqueda) {
      ...
      filtradas = supervisoresGlobal.filter(
        (sup) =>
  ...
  // --- MODAL DE ADVERTENCIA PARA ELIMINAR SUPERVISOR ---
  let idSupervisorAEliminar = null;
      ...
      if (idSupervisorAEliminar) {
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(
              errorData.error || "Error al eliminar el supervisor"
            );
          }
