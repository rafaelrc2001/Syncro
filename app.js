...
app.listen(PORT, () => {
  ...
  console.log(
    'Diagnóstico disponible en: /api/diagnostico'
  );
});
...
app.get('/api/areas/:id', async (req, res) => {
  ...
  try {
    const result = await db.query(
      'SELECT id_area as id, nombre FROM areas WHERE id_area = $1',
      [id]
    );
...
app.post('/api/areas', async (req, res) => {
  ...
  try {
    const query =
      'INSERT INTO areas(nombre) VALUES($1) RETURNING id_area as id, nombre';
...
app.get('/api/supervisores', async (req, res) => {
  try {
    const result = await db.query(
      'SELECT id_supervisor as id, nombre, correo, extension FROM supervisores'
    );
    res.json(result.rows);
  } catch (err) {
