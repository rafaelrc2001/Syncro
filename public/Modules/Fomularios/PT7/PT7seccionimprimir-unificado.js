...
if (btnRegresar) {
  btnRegresar.addEventListener("click", function () {
    ...
    if (window.location.pathname.includes("PT7imprimir2.html")) {
      window.location.href = "/Modules/Usuario/AutorizarPT.html";
    } else if (window.location.pathname.includes("PT7imprimirseg.html")) {
      window.location.href = "/Modules/SupSeguridad/SupSeguridad.html";
    } else if (window.location.pathname.includes("PT7imprimirsup.html")) {
        ...
        radios.forEach((name) => {
          if (permiso[name]) {
            ...
            radios.forEach((radio) => {
              if (radio.value === permiso[name]) radio.checked = true;
            });
          }
        });
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (btnGuardar) {
    btnGuardar.addEventListener("click", async function () {
      ...
      try {
        const response = await fetch(`/api/pt-radiacion/${idPermiso}`, {
          method: "PUT",
          headers: {
        ...
        if (response.ok) {
          ...
          console.log("Requisitos guardados:", result);
          alert("Requisitos actualizados con éxito");
        } else {
        ...
        alert(
          "Error al guardar requisitos. Revisa la consola para más detalles."
        );
      }
    });
  }
  ...
  function mostrarAST(ast) {
    ...
    if (eppList) {
      ...
      if (ast && ast.epp_requerido) {
        const epp = ast.epp_requerido.split(",");
        epp.forEach((item) => {
    ...
    if (maqList) {
      ...
      if (ast && ast.maquinaria_herramientas) {
        const maq = ast.maquinaria_herramientas.split(",");
        maq.forEach((item) => {
          const li = document.createElement("li");
          li.textContent = item.trim();
    ...
    if (matList) {
      ...
      if (ast && ast.material_accesorios) {
        const mat = ast.material_accesorios.split(",");
        mat.forEach((item) => {
          const li = document.createElement("li");
          li.textContent = item.trim();
    ...
    fetch(`/api/verformularios?id=${encodeURIComponent(idPermiso2)}`)
      ...
      .then((data) => {
        ...
        if (data && data.data) {
          ...
          setText("prefijo-label", general.prefijo || permiso.prefijo || "-");
          setText("empresa-label", general.empresa || permiso.empresa || "-");
          setText(
            "nombre-solicitante-label",
            general.solicitante || permiso.solicitante || "-"
          );
          setText(
            "sucursal-label",
            general.sucursal || permiso.sucursal || "-"
          );
          setText(
            "contrato-label",
            general.contrato || permiso.contrato || "-"
          );
          setText("plant-label", general.area || permiso.area || "-");
          setText(
            "start-time-label",
            general.hora_inicio || permiso.hora_inicio || "-"
          );
          setText("fecha-label", general.fecha || permiso.fecha || "-");
          setText(
            "work-order-label",
            general.ot_numero || permiso.ot_numero || "-"
          );
          setText("tag-label", general.tag || permiso.tag || "-");
          setText(
            "descripcion-trabajo-label",
            general.descripcion_trabajo || permiso.descripcion_trabajo || "-"
          );
          setText(
            "equipment-description-label",
            general.equipo_intervenir || permiso.equipo_intervenir || "-"
          );
          setText(
            "maintenance-type-label",
            general.tipo_mantenimiento || permiso.tipo_mantenimiento || "-"
          );
          // Campos adicionales
          setText(
            "special-tools-label",
        ...
        if (data.ast) {
          ...
          console.log("No hay datos de AST en la respuesta");
        }
        if (data.actividades_ast) {
          mostrarActividadesAST(data.actividades_ast);
        } else {
          console.log("No hay actividades AST en la respuesta");
        }
        if (data.participantes_ast) {
          mostrarParticipantesAST(data.participantes_ast);
        } else {
          console.log("No hay participantes AST en la respuesta");
        }
        let prefijo =
    ...
    .then((result) => {
      ...
      if (result.success && result.data) {
        ...
        filas.forEach((fila) => {
          ...
          tdNombre.textContent = fila.nombre || "N/A";
          const tdCargo = document.createElement("td");
          tdCargo.textContent = fila.cargo;
